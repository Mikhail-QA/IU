...
@allure.story("Проверка наличия элементов в поп-апе Оставить отзыв для не авторизованного пользователя")
class CheckPopupReview(StartInterneturokClassMethod):
    @allure.step("Нажать на кнопку Оставить отзыв")
    def test_001_open_popup_review(self):
        self.driver.find_element_by_css_selector("a.review__button").click()

    @allure.step("В Header поп-апе Оставить отзыв отображается текст (Спасибо, что решили написать!)")
    ...
    @allure.step("В Header поп-апе Оставить отзыв отображается кнопка Крестик(закрыть)")
    def test_003_look_button_close(self):
        self.assertTrue(self.is_element_present(By.CLASS_NAME, "popup-header__close"))

    @allure.step("Блок body-text http://prntscr.com/imh2r1")
    ...
    @allure.step("Отображается поле (Ваше имя)")
    ...
    @allure.step("Отображается поле (E-mail)")
    ...
    @allure.step("Отображается поле (Текст отзыва)")
    ...
    @allure.step("Отображается поле (Откуда вы о нас узнали?)")
    ...
    @allure.step("Отображается кнопка Отправить")
