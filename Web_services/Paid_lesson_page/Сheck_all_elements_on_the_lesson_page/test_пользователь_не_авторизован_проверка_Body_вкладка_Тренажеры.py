...
class ChecksAllElementsInLessonPageTheBodyTabTrainersUserNotAuth(StartInterneturokClassMethod):
    ...
    @allure.step("Во вкладке Тренажер отображается текст (Закрепите материал с помощью тренажёров)")
    def test_title_text_trainers(self):
        self.assertEqual("Закрепите материал с помощью тренажёров",
                         self.driver.find_element_by_css_selector("p.practices-desc").text)

    @allure.step("Во вкладке Тренажеры отображается заглушка (Тренажеры доступны по абонементу)")
    def test_video_blocker(self):
        ...
        with allure.step("В заглушка присутствует текст (Тренажеры доступны по абонементу)"):
            self.assertEqual("Тренажеры доступны по абонементу",
    ...
    def test_button_trainers_one(self):
        ...
        with allure.step("В блоке тренажеров урока отображается текст (Тренажер 1)"):
            self.assertEqual("Тренажёр 1", self.driver.find_element_by_css_selector(
